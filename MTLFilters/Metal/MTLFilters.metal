...
typedef enum BlurTextureIndex
{
    BlurTextureIndexInput  = 0,
    BlurTextureIndexOutput = 1,
} BlurTextureIndex;

typedef enum BlurBufferIndex
{
    BlurBufferIndexLOD = 0,
} BlurBufferIndex;
...
kernel void gaussianblurHorizontal(texture2d<half, access::read>  inTexture   [[ texture(BlurTextureIndexInput) ]],
                                   texture2d<half,  access::write> outTexture  [[ texture(BlurTextureIndexOutput) ]],
                                   constant uint                 &lod     [[ buffer(BlurBufferIndexLOD) ]],
                                   uint2                          gid         [[ thread_position_in_grid ]])
{
    gaussianblur(inTexture, outTexture, lod, int2(1, 0), gid);
}

kernel void gaussianblurVertical(texture2d<half, access::read>  inTexture   [[ texture(BlurTextureIndexInput) ]],
                                 texture2d<half, access::write> outTexture  [[ texture(BlurTextureIndexOutput) ]],
                                 constant uint                 &lod    [[ buffer(BlurBufferIndexLOD) ]],
                                 uint2                          gid         [[ thread_position_in_grid ]])
{
